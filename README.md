...
| **共通近傍語探索** | 複数の単語を入力し、すべてに意味的に近い「共通の単語空間」を探索する（例: 夕暮れ＋廃墟＋静寂 → 共通の雰囲気を持つ単語群） |
...
| POST | `/cluster` | 複数単語の重心ベクトルに近い共通近傍語を返す |
...
| パラメータ | デフォルト | 説明 |
|---|---|---|
| `limit` | 50 | 返す件数（最大 200） |
| `offset` | 0 | スキップする件数（ページング用） |
| `q` | （なし） | 絞り込み文字列。`scene_ja` と `prompt` を対象に部分一致検索 |
...
```json
{
  "entries": [
    {
      "ts": "2026-04-12T15:30:00+09:00",
      "pivot": "孤独",
      "mode": "combo",
      "scene_ja": "霧の中に佇む廃駅",
      "prompt": "abandoned train station in fog, ..."
    }
  ],
  "total": 42
}
...
- `words`: 共通近傍語を探索したい単語リスト（2語以上必須）
- `topn`: 返す単語の数（デフォルト 10）
- 入力単語自身は結果から除外される
...
```json
{
  "results": [["廃墟", 0.87], ["薄明かり", 0.83], ["廃墟群", 0.81], ...]
}
```

複数単語の重心ベクトル（centroid）に意味的に近い単語を返す。入力単語すべてのニュアンスを共有するような語が得られる。
